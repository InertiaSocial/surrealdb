...
async fn import(
	...
	let comment_idiom = headers
		.get("wasm-description")
	...
	match db.process(DefineStatement::Wasm(wasm_def).into(), &session, None).await {
		Ok(_) => {
			info!(
				"WASM module '{}' version '{}' (hash: {}) stored and defined successfully",
				name_str, version, hash
			);
			Ok(output::none())
		}
		Err(e) => {
			warn!(
				"Failed to define WASM module '{}' after storing: {}. Cleaning up...",
				name_str, e
			);
...
async fn export(
	...
	let wasm_def = match tx.get_db_wasm(&nsv, &dbv, &name, &version).await {
		...
		Err(e) => {
			...
			if let surrealdb::err::Error::WasmNotFound {
				..
			} = e
			{
				return Err(Error::Other(format!(
					"WASM module '{}' version '{}' not found",
					name, version
				)));
			}
	...
	match obs::get(&path).await {
		Ok(data) => {
			info!(
				"Retrieved WASM module '{}' version '{}' (hash: {})",
				name, version, wasm_def.hash
			);
		...
		Err(e) => Err(Error::Other(format!("Failed to retrieve WASM module binary: {}", e))),
	}
}
...
fn is_valid_wasm_module(data: &[u8]) -> bool {
	println!("{:?}", data.iter().take(10).map(|b| format!("{:02X}", b)).collect::<Vec<String>>());
	...
	let _version_bytes_standard = [0x01, 0x00, 0x00, 0x00]; // Standard WASM: Version 1
	let version_bytes_component = [0x0D, 0x00, 0x01, 0x00]; // Component Model: wasip2

	data.starts_with(&magic) && data[4..8] == version_bytes_component
}
